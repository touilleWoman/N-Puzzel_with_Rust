...
fn solution_found(open_total: u32, open_now: u32, cur: &Matrix, row: i32) -> Vec<i32> {
    ...
    let mut count: u16 = 0;
    recursive_print_parent(cur, row, & mut count);
    println!("Nb of moves: {}", count - 1);
    return cur.data.clone();
}


fn recursive_print_parent(cur: &Matrix, row:i32, count: &mut u16){
    *count +=1;
